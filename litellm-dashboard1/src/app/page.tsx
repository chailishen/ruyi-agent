...
function CreateKeyPageContent() {
  ...
  useEffect(() => {
    if (redirectToLogin) {
      storeReturnUrl();
      // Prototype: stay on Next dev server (e.g. :3000/login), not LiteLLM Proxy (:4000)
      const baseLoginUrl = isMockMode()
        ? `${window.location.origin}/login`
        : `${proxyBaseUrl || ""}/ui/login`;
  ...
  useEffect(() => {
    if (!authLoading && isLegacyRedirect) {
      const base = isMockMode() ? "" : `${proxyBaseUrl || ""}/ui`;
